...
main ()
{
  ...
  logonHeaderPart2 << "35=A" << SOH << "49=CLIENT1" << SOH <<
  "56=EXECUTOR" << SOH << "34=1" << SOH << "52=" << reqFormatDateTime << SOH << "98=0" << SOH << "108=30" << SOH;
  ...
  long idx;
  unsigned int cks;
  for( idx = 0L, cks = 0; idx < logonHeaderPart1.str().length(); cks += (unsigned int)logonHeaderPart1.str().c_str()[ idx++ ] );
