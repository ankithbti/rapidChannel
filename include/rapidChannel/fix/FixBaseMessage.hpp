		...
		_isValidMessage(true)
{
	...
	{
		return _header.getLength(beginString, bodyLen, checksum) + getLength(beginString, bodyLen, checksum)
				+ _footer.getLength(beginString, bodyLen, checksum);
	}

	int checksum(int checksum = 10) const
	{
		std::cout << "Total bytecount: " << _header.getTotal(checksum) << " " << getTotal(checksum) << " "
				<< _footer.getTotal(checksum) << std::endl;
		return ((_header.getTotal(checksum) + getTotal(checksum) + _footer.getTotal(checksum)) % 256);
	}
